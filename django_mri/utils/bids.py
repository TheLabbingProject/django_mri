...
class BidsManager:
    ...
    * `The BIDS Specification`_.
    ...
    def build_bids_path(self, scan):
        ...
        """
        self._logger.debug(
            f"Building BIDS-compatible path for scan #{scan.id}"
        )
        ...
        if default_bids_path is None:
            raise ValueError(
                "BIDS path could not be derived from scan parameters."
            )
        self._logger.debug(
            f"Default relative path generated by dicom_parser based on DICOM metadata:\n{default_bids_path}"  # noqa: E501
        )
        self._logger.debug("Replacing Patient ID with subject primary key...")
        ...
        self._logger.debug(f"Result: {fixed_relative_path}")
        bids_path = get_bids_dir() / fixed_relative_path
        self._logger.debug(f"Checking for an existing file at {bids_path}")
        path_with_suffix = {
            bids_path.with_suffix(".nii"),
            bids_path.with_suffix(".nii.gz"),
        }
        try:
            ...
            try:
                existing_run_label = re.findall(
                    self.RUN_PATTERN, str(bids_path)
                )[0]
            except IndexError:
                self._logger.debug("No existing run label found.")
                self._logger.debug(f"Renaming {scan.id} to include run label.")
                existing_run_label = self.RUN_LABEL_TEMPLATE.format(index=1)
